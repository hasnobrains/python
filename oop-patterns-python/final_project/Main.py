...
def create_game(sprite_size, is_new):
    ...
    if is_new:
        img = os.path.join("texture", "Hero.png")
        sprite = Service.create_sprite(img, sprite_size)
        hero = Objects.Hero(base_stats, sprite)
        engine = Logic.GameEngine()
        Service.service_init(sprite_size)
        Service.reload_game(engine, hero)
        with ScreenEngine as SE:
            screen_handle = SE.ScreenHandle((0, 0))
            help_window = SE.HelpWindow((700, 500), pygame.SRCALPHA, (0, 0),
                                        screen_handle)
            info_window = SE.InfoWindow((160, 600), (50, 50),
                                        help_window)
            progress_bar = SE.ProgressBar((640, 120), (640, 0),
                                          info_window)
        ...
        img = os.path.join("texture", "Hero.png")
        hero.sprite = Service.create_sprite(img, sprite_size)
